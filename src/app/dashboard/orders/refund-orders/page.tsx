  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        if (data) {
          const refundOrders = data.filter((order) =>
            ["RequestRefund", "AcceptedRefund", "RejectedRefund", "Refunded"].includes(order.status)
          );
          setOrders(refundOrders);
          setFilteredOrders(refundOrders);
        } else {
    ...
    <div>
      <div className="flex justify-evenly">
        ...
        <Space className="flex-col" style={{ marginBottom: 16 }}>
          <Typography.Title level={2}>Refund Requests</Typography.Title>
