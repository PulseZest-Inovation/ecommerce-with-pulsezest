...
const OrderDetails: React.FC<OrderDetailsProps> = ({
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const updateCustomerOrderStatus: boolean = await updateDocWithCustomId(
      ...
      const emailRequestBody = {
        ...
        emailType: status, // Email type must match the backend handler's logic
      };

      const emailResponse = await fetch(
        `http://localhost:3001/api/send-email`, // Use env variable for API URL
        {
          method: "POST",
          headers: {
      ...
      console.log(emailResponse.json());
